const { accountId, blockHeight, Config, Post, key, communities } = props;
const update = (s) => State.update(s);
const refresh = (ms) => setTimeout(() => update({ rd: Math.random() }), ms);

const community = communities.find((c) => c.id === key);

State.init({});

// -- Filters
if (Array.isArray(community.rules.SBTs)) {
  // user should have the SBTs
  let isValid = false;

  community.rules.SBTs.forEach((SBT) => {
