...
function daysAndHoursToMilliseconds(days, hours) {
...
function createBounty() {
  ...
  const amountInYocto = Big(amount)
    .mul(Big(10).pow(metadata.decimals))
    .toFixed();
  const deadline = daysAndHoursToMilliseconds(
    deadlineDays,
    deadlineHours
  ).toFixed();
  ...
  Near.call([
    {
      ...
      args: {
        proposal: {
          description: `New Bounty proposed by ${context.accountId}`,
          kind: {
            AddBounty: {
              bounty: {
...
useEffect(() => {
  if (token) {
    Near.asyncView(token, "ft_metadata", {}).then((ftMeta) => {
      setFtMetadata(ftMeta);
    });
  }
}, [token]);
    ...
    <div className="container-fluid">
      <div className="row gy-4">
        <div className="col-sm-6">
          <div className="text-muted text-sm mb-1">DAO</div>
          <Widget
        ...
        <div className="col-sm-6">
          <div className="text-muted text-sm mb-1">Proposer</div>
          <Widget
            src="near/widget/AccountProfile"
            props={{
              accountId,
        ...
        <div className="form-group col-sm-6">
          <label>No of times the bounty can be claimed</label>
          <input
            type="number"
            value={claims}
        ...
        <div className="form-group col-sm-6">
          <label>Deadline (time after user claim the bounty)</label>
          <div className="deadline-input d-flex gap-2">
            <div class="input-group">
              <input
                ...
                min="0"
              />
              <span class="suffix text-muted">days</span>
            </div>
            <div class="input-group">
              <input
                type="number"
                ...
                min="0"
              />
              <span class="suffix text-muted">hours</span>
            </div>
          </div>
        </div>
